...
public interface ConferenceRepository extends JpaRepository<Page, Long> {
  ...
  @Query("SELECT page FROM Page page JOIN page.events event WHERE page.pageType = 'CONFERENCE' AND DATE(event.date) >= CURRENT_DATE() AND (page.state = 'PUBLISHED' OR page.state = 'BLOCKED') AND page.visibility = 'PUBLIC'")
  ...
  @Query("SELECT DISTINCT page FROM Page page LEFT JOIN page.invitations invitation LEFT JOIN page.administrators administrator JOIN page.events event WHERE page.pageType = 'CONFERENCE' AND (((page.visibility = 'PUBLIC' OR page.visibility = 'AUTHENTICATED') OR (page.visibility = 'INVITED' AND invitation.user.id = :user_id)) AND (page.state = 'PUBLISHED' OR page.state = 'BLOCKED') AND DATE(event.date) >= CURRENT_DATE() OR (administrator.user.id = :user_id AND page.state != 'DELETED'))")
