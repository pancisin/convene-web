...
//	<S extends Event> S save(S entity);
	...
	@Query("SELECT DISTINCT event FROM Event event LEFT JOIN event.page page LEFT JOIN page.administrators administrator LEFT JOIN event.page page LEFT JOIN page.administrators cAdmin WHERE (event.state = 'PUBLISHED' OR event.owner.id = :userId OR administrator.user.id = :userId OR cAdmin.user.id = :userId) AND DATE(event.date) = DATE(:date)")
	...
	@Query("SELECT event FROM User user JOIN user.events event WHERE user.id = :userId AND (event.state = 'PUBLISHED' OR event.owner.id = :userId) AND DATE(event.date) = DATE(:date) AND event.page IS NULL")
