...
//	@Cacheable("pages")
  ...
  @Query("SELECT page FROM Page page JOIN page.branch branch WHERE page.pageType = 'PAGE' AND branch.category.id = :category_id AND (page.state = 'PUBLISHED' OR page.state = 'BLOCKED')")
	...
	@Query("SELECT page FROM PageMember pageMember JOIN pageMember.user user JOIN pageMember.page page WHERE pageMember.active IS TRUE AND page.pageType = 'PAGE' AND user.id = :user_id AND (page.state = 'PUBLISHED' OR page.state = 'BLOCKED')")
