...
public interface MessageRepository extends JpaRepository<Message, Long> {
	...
	@Query("SELECT message FROM Message message WHERE (message.sender.id = :user_id AND message.recipient.id = :participant_id) OR (message.recipient.id = :user_id AND message.sender.id = :participant_id) ORDER BY message.created ASC")
...
//	public List<Message> getPublic(@Param("company") Company company);
	...
	@Query("SELECT message FROM Message message WHERE message.recipient.id IS NULL AND message.sender.id IN (:users) ORDER BY message.created ASC")
